...
Axiom fmu_subset_of_mu : (set.Set.subset (f (intersect (comprehension fc3)))
  (intersect (comprehension fc4))).

Parameter fc5: ((set.Set.set t) -> bool).

Parameter fc6: ((set.Set.set t) -> bool).

Axiom fc_def5 : forall (x:(set.Set.set t)), ((fc5 x) = true) <->
  (set.Set.subset (f x) x).

Axiom fc_def6 : forall (x:(set.Set.set t)), ((fc6 x) = true) <->
  (set.Set.subset (f x) x).

(* Why3 goal *)
Theorem mu_subset_of_fmu : (set.Set.subset (intersect (comprehension fc5))
  (f (intersect (comprehension fc6)))).
